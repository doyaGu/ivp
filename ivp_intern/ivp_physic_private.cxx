...
void IVP_Cluster_Manager::fire_event_object_deleted(IVP_Event_Object *event_obj)
{
	...
	if (obj_table != NULL)
	{
		...
		for (i = obj_table->listeners.len() - 1; i >= 0; i--)
		{
			...
			if (i > 0 && !obj_callback_hash->find_table(ro))
...
void IVP_Cluster_Manager::fire_event_collision_object_deleted(IVP_Real_Object *real_object)
{
	...
	if (obj_table != NULL)
	{
		...
		{
			IVP_Listener_Collision *lis = obj_table->listeners.element_at(i);
			if (!(lis->get_enabled_callbacks() & IVP_LISTENER_COLLISION_CALLBACK_OBJECT_DELETED))
				continue;
...
IVP_Real_Object *IVP_Cluster_Manager::get_next_real_object_in_cluster_tree(IVP_Object *object)
{
	...
	for (next = next_object; !next; next = object->next_in_cluster)
	{
		...
		if (!object)
		{
			IVP_Cluster *root = get_root_cluster();
			if (root == old_father)
				return (NULL); /* necessary to avoid endless loop caused by the last object
...
void IVP_Cluster_Manager::check_for_unused_objects(IVP_Universe_Manager *um)
{
		...
		return;
	{
		...
		for (; i >= 0; i--)
		{
			...
			if (an_object->get_collision_check_reference_count() != 0)
				continue;
		...
		return;
	{
		...
		for (; i >= 0; i--)
		{
			...
			if (!an_object)
				return; // no objects available
			this->an_object_to_be_checked = get_next_real_object_in_cluster_tree(this->an_object_to_be_checked);
			if (an_object->get_collision_check_reference_count() != 1)
				continue;
			um->object_no_longer_needed(an_object);
			if (number_of_real_objects < ums->num_objects_in_environment_threshold_1)
				return;
...
void IVP_Core::debug_vec_movement_state()
{
	...
	IVP_IF(0)
	{
		...
		out_text = p_make_string("oob%lx_sp%.3f", (intp)one_object & 0x0000ffff, one_object->speed.real_length()); //,one_object->get_energy_on_test(&one_object->speed,&one_object->rot_speed));
		if (my_core->movement_state == IVP_MT_CALM)
		{
			// out_text=p_export_error("%lxo_calm%lx",(intp)one_object&0x0000ffff,(intp)fr_i&0x0000ffff);
			// sprintf(out_text,"%ldobj_calm%lx",counter,(intp)one_object);
			v_color = 3;
		}
		else
		{
			...
			{
				// out_text=p_export_error("%lxdo_not_m%lx",(intp)one_object&0x0000ffff,(intp)fr_i&0x0000ffff);
				v_color = 6;
			}
			else
			{
				// out_text=p_export_error("%lxdo_moved%lx",(intp)one_object&0x0000ffff,(intp)fr_i&0x0000ffff);
				// sprintf(out_text,"%obj_moved%lx",(intp)one_object);
				v_color = 1;
			}
		}
