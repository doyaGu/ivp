...
inline void IVP_Calc_Next_PSI_Solver::calc_rotation_matrix(IVP_FLOAT delta_sim_time, IVP_U_Quat *q_core_f_core)
{
    ...
    IVP_U_Point diff_other_rot_inertia_div_this((ri->k[1] - ri->k[2]) * iri->k[0],
...
void IVP_Calc_Next_PSI_Solver::calc_next_PSI_matrix(IVP_Event_Sim *event_sim, IVP_U_Vector<IVP_Hull_Manager_Base> *active_hull_managers_out)
{
    IVP_IF(1)
    {
        ...
        {
            fprintf(dm->out_deb_file, "making_calc_next_psi %x at %f\n", (int)(0x0000ffff & (intp)this), core->environment->get_current_time().get_time());
        }
    }

    if (!(core->car_wheel && (core->max_surface_deviation == 0.0f)))
    { //@@CB
