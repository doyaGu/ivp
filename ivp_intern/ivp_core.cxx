...
{
    IVP_Time time = environment->get_current_time();
    // Cache time computation in variable
    IVP_DOUBLE t = time.get_time();
...
void IVP_Core::synchronize_with_rot_z()
{
    ...
    q_core_f_core.set_invert_mult(&q_world_f_core_last_psi, &q_world_f_core_next_psi);
...
IVP_Movement_Type IVP_Core::calc_movement_state(IVP_Time psi_time)
{
    ...
    while (1)
    { // break if object has moved
        IVP_DOUBLE qdist = pos_world_f_core_last_psi.quad_distance_to(&position_world_f_core_calm_reference[0]);
        if (qdist > IVP_MINIMAL_TRANS_SPEED_PER_PSI_SHORT * IVP_MINIMAL_TRANS_SPEED_PER_PSI_SHORT)
            break;

        IVP_DOUBLE qrot_sum = q_world_f_core_next_psi.inline_estimate_q_diff_to(&q_world_f_core_calm_reference[0]);
        if (qrot_sum * upper_limit_radius * upper_limit_radius > IVP_MINIMAL_ROT_SPEED_PER_PSI_SHORT * IVP_MINIMAL_ROT_SPEED_PER_PSI_SHORT)
            break;
    ...
    { // break if object has moved
        ...
        if (q_dist > IVP_MINIMAL_TRANS_SPEED_PER_PSI_LONG * IVP_MINIMAL_TRANS_SPEED_PER_PSI_LONG)
            break;

        IVP_DOUBLE qrot_sum = q_world_f_core_last_psi.inline_estimate_q_diff_to(&q_world_f_core_calm_reference[1]);
        if (qrot_sum * upper_limit_radius * upper_limit_radius > IVP_MINIMAL_ROT_SPEED_PER_PSI_LONG * IVP_MINIMAL_ROT_SPEED_PER_PSI_LONG)
            break;
