...
IVP_DOUBLE IVP_Impact_Solver::get_total_energy()
{
	...
	IVP_DOUBLE ener;
	ener = core[0]->get_energy_on_test(&trans_speed[0], &rot_speed[0]) + core[1]->get_energy_on_test(&trans_speed[1], &rot_speed[1]);
...
void IVP_Impact_Solver::do_impact(IVP_Core *pushed_cores[2], IVP_BOOL allow_delaying, int pushes_while_system, IVP_FLOAT rescue_speed_addon)
{
	...
	used_conservation = 1.0f - (1.0f / (pushes_while_system * IVP_INV_HALF_CONSERVATION_STEPS + 1.0f)) * (1.0f - percent_energy_conservation);
	...
	if (core[1]->physical_unmoveable || core[1]->pinned)
	{ //@@CBPIN
...
void IVP_Impact_Solver_Long_Term::do_impact_long_term(IVP_Core *pushed_cores[2], IVP_FLOAT rescue_speed_val, IVP_Contact_Point *cp)
{
	...
	{
		if (contact_core[0])
			contact_core[0]->core_plausible_check();
		if (contact_core[1])
			contact_core[1]->core_plausible_check();
	}
...
void IVP_Impact_System::impact_system_check_start_pair(IVP_Friction_Core_Pair *start_pair, IVP_Contact_Point *mdfr)
{
	...
	for (int i = start_pair->fr_dists.len() - 1; i >= 0; i--)
	{
		...
		if (mdfr != my_dist)
		{ // warning old version checked mdfr !=  next_dist
...
void IVP_Impact_System::init_and_solve_impact_system(IVP_Mindist *mindist, IVP_Friction_System *fs_system, IVP_Friction_Core_Pair *start_pair, IVP_Contact_Point *start_fr_dist)
{
	...
	if (start_core0 && !start_core0->physical_unmoveable && !start_core0->pinned)
	{ //@@CBPIN
	...
	if (!core0->physical_unmoveable && !core0->pinned)
	{ //@@CBPIN
		add_known_core_to_impact_system(core0);
	}

	if (start_core1 && !start_core1->physical_unmoveable && !start_core1->pinned)
	{ //@@CBPIN
	...
	if (worst_impact_dist)
	{
...

		{
			for (int i = 1; i >= 0; i--)
			{
				...
				if (core && !core->physical_unmoveable && !core->pinned)
				{ //@@CBPIN
...
void IVP_Impact_Solver::get_world_direction_second_friction(IVP_Contact_Point *cp)
{ // IVP_U_Float_Point *ret_second_friction_vec,IVP_FLOAT *sin_second_friction) {
	IVP_Material *mtl[2];
