...
void IVP_Friction_Solver::solve_linear_equation_and_push(IVP_Friction_System *my_fs,
                                                         ...
                                                         IVP_U_Memory *mem_friction)
{
    ...
    actives_matrix.desired_vector =
        (IVP_DOUBLE *)mem_friction->get_mem(actives_matrix.aligned_row_len * sizeof(IVP_DOUBLE));
    actives_matrix.result_vector =
        (IVP_DOUBLE *)mem_friction->get_mem(actives_matrix.aligned_row_len * sizeof(IVP_DOUBLE));
    actives_matrix.matrix_values = (IVP_DOUBLE *)mem_friction->get_mem(
        (total_actives * actives_matrix.aligned_row_len + IVP_VECFPU_SIZE - 1) * sizeof(IVP_DOUBLE));
    actives_matrix.align_matrix_values();

    memset((char *)dist_change_mat.result_vector,
           0,
    ...
    {
        if ((test_gauss_solution_suggestion(
                 actives_matrix.result_vector, active_is_at_pos, total_actives, mem_friction) == IVP_OK))
        {
            IVP_IF(1)
            {
                this->gauss_succed++;
    ...
    if (constraint_unilateral.init_and_solve_lc(dist_change_mat.matrix_values,
                                                dist_change_mat.desired_vector,
                                                dist_change_mat.result_vector,
...
void IVP_Friction_System::do_pushes_distance_keepers(const IVP_Event_Sim *es_in)
{
         ...
         fr_dist = this->get_next_friction_dist(fr_dist))
    {
        ...
        IVP_FLOAT speedup;
        speedup = ((IVP_FLOAT)(IVP_SLOWLY_TURN_ON_KEEPER - fr_dist->slowly_turn_on_keeper) * (IVP_FLOAT(1.0f / IVP_SLOWLY_TURN_ON_KEEPER) * es_in->i_delta_time));
        ...
        if (impulse > 0.0f)
        {
            ...
            // printf("%x gap %f single old %f new %f  impulse %f\n", this, gap_diff, closing_speed,
            // closing_speed2, impulse);
        }
        else
        {
            fr_dist->now_friction_pressure = 0.0f;
         ...
         m_index++, my_dist = fs->get_next_friction_dist(my_dist))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                my_dist->has_negative_pull_since++;
                if (my_dist->has_negative_pull_since > 9)
                {                                         // allow only 9 negative pushes in following PSIs
...
void IVP_Friction_Solver::print_dist_velocity(IVP_Friction_System *fs)
{
    ...
    while (my_dist)
    {
        ...
        core0->get_surface_speed_on_test(&my_dist->tmp_contact_info->contact_point_cs[0],
                                         &speed_var_trans,
                                         &speed_var_rot,
                                         &w_speed0);
        speed_var_trans.add(&core1->speed, &core1->speed_change);
        speed_var_rot.add(&core1->rot_speed, &core1->rot_speed_change);
        core1->get_surface_speed_on_test(&my_dist->tmp_contact_info->contact_point_cs[1],
                                         &speed_var_trans,
                                         &speed_var_rot,
                                         &w_speed1);
...

        {
            ...
            for (int n = fr_pairs_of_objs.len() - 1; n >= 0; n--)
            {
                ...
                for (int m = fr_pair->fr_dists.len() - 1; m >= 0; m--)
                {
                    ...
                    int found_mine = 0;
                    for (IVP_Contact_Point *mindist2 = fs->get_first_friction_dist(); mindist2;
                         mindist2 = fs->get_next_friction_dist(mindist2))
                    {
                        if (mindist2 == fr_dist)
                        {
                            found_mine = 1;
...
void IVP_Friction_Solver::setup_coords_mindists(IVP_Friction_System *fs)
{
         ...
         my_dist = fs->get_next_friction_dist(my_dist))
    {
        ...
        info->friction.friction_infos[1] =
            my_dist->get_synapse(1)->get_object()->get_core()->get_friction_info(fs);
...
/* friction distance with number 'current_contact_point_index' is tested. Therefore both objects of
 * distance get a test push Velocity changes are represented with 'rotation_vec' (obj coords) and
 * 'translation_vec' (world coords) then this funcion is called for both objects for one object is
 * looked if other distances are inflicted (speed of distance changes) if yes, speed change is
 * represented in a line of the matrix, with line number = number of the distance values are added
 * because a column of the matrix is built with testing of two objects (example: diagonal elements,
 * but not only)
 */
                                                      ...
                                                      IVP_U_Float_Point *translation_vec)
{
    ...
    // dist_counter is number of distance that is tested. It is represented with a column of the
    // matrix.
    ...
    for (int col = 0; col < fr_info->friction_springs.n_elems; col++)
    {
        ...
        int which_obj =
            1; // a distance is between two objects, which one is mine ? - 0 first, 1 second
        IVP_DOUBLE factor;
        ...
        if (info->contact_core[which_obj])
        {
            IVP_DOUBLE speed_change_scalar =
                factor * get_closing_speed_core_i(info, which_obj, rotation_vec, translation_vec);
            dist_change_mat.matrix_values[line_num * dist_change_mat.aligned_row_len + current_contact_point_index] += speed_change_scalar;
            // if ( line_num != current_contact_point_index){
            // dist_change_mat.matrix_values[current_contact_point_index * dist_change_mat.columns +
            // line_num] += speed_change_scalar * ( ivp_rand() * 0.001f + 1.0f);
            // }
        }
    }
}
...
int IVP_Friction_Solver::calc_solver_PSI(IVP_Friction_System *fs, int *active_is_at_pos)
{
    ...
    {
        if (my_dist->get_lt()->index_in_fs < 0)
            continue;
...
void IVP_Friction_System::reorder_mindists_for_complex()
{
    ...
    while (my_frdist)
    {
        ...
        if (1 && ((my_frdist->get_gap_length() >= ivp_mindist_settings.max_distance_for_friction) || (my_frdist->get_lt()->friction_is_broken == IVP_TRUE)))
        {
...
void IVP_Friction_System::exchange_friction_dists(IVP_Contact_Point *first_frd,
                                                  IVP_Contact_Point *second_frd)
{
...
void IVP_Friction_System::bubble_sort_dists_importance()
{
    ...
    IVP_IF(1)
    {
             ...
             first_dist = get_next_friction_dist(first_dist))
        {
            ...
            {
                IVP_ASSERT(
                    !(first_dist->has_negative_pull_since > second_dist->has_negative_pull_since));
            }
        }
    }
}
...
// for nonterminal objects: turn on fast piling but delete whole energy (with delayed pushes the
// energy would be accumulated)
...
void IVP_Friction_System::do_friction_system(const IVP_Event_Sim *es_in)
{
    ...
    this->reorder_mindists_for_complex(); // get rid of mindist already heaving a big gap @+@
                                          // ->check speed after do_pushes_distance_keepers also
