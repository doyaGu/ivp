...
public:
     ...
     *	Description:	gets the radius and the max( length( surface_normal cross_product
     *surface_point )) if radius_deviation can not be calculated, set it to radius
     ********************************************************************************/
     ...
     *                  returns all ledges which intrude into a sphere around the observer's
     *position Note:		Polygons only
     ********************************************************************************/
    ...
    {
        if (observer_position_object->quad_length() > radius * radius)
            return;
...
IVP_Ball::IVP_Ball(IVP_Cluster *father, const IVP_Template_Ball *tball, const IVP_Template_Real_Object *templ, const IVP_U_Quat *rotation, const IVP_U_Point *position) : IVP_Real_Object(father, NULL, templ, rotation, position)
{
