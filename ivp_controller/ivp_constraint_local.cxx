...
#if defined(LINUX) || (defined(__MWERKS__) && defined(__POWERPC__))
#include <alloca.h>
#endif
...
void IVP_Constraint_Local::sort_translation_mapping()
{
	...
	if (fixed[0] == IVP_CONSTRAINT_AXIS_FIXED)
		fixedtrans_dim++;
	if (fixed[0] & IVP_CONSTRAINT_AXIS_LIMITED)
		limitedtrans_dim++;
	if (fixed[1] == IVP_CONSTRAINT_AXIS_FIXED)
		fixedtrans_dim++;
	if (fixed[1] & IVP_CONSTRAINT_AXIS_LIMITED)
		limitedtrans_dim++;
	if (fixed[2] == IVP_CONSTRAINT_AXIS_FIXED)
		fixedtrans_dim++;
	if (fixed[2] & IVP_CONSTRAINT_AXIS_LIMITED)
		limitedtrans_dim++;
...
void IVP_Constraint_Local::sort_rotation_mapping()
{
	...
	if (fixed[3] == IVP_CONSTRAINT_AXIS_FIXED)
		fixedrot_dim++;
	if (fixed[3] & IVP_CONSTRAINT_AXIS_LIMITED)
		limitedrot_dim++;
	if (fixed[4] == IVP_CONSTRAINT_AXIS_FIXED)
		fixedrot_dim++;
	if (fixed[4] & IVP_CONSTRAINT_AXIS_LIMITED)
		limitedrot_dim++;
	if (fixed[5] == IVP_CONSTRAINT_AXIS_FIXED)
		fixedrot_dim++;
	if (fixed[5] & IVP_CONSTRAINT_AXIS_LIMITED)
		limitedrot_dim++;
	matrix_size = fixedtrans_dim + fixedrot_dim + limitedtrans_dim + limitedrot_dim;
}
...
void IVP_Constraint_Local::do_simulation_controller(IVP_Event_Sim *es, IVP_U_Vector<IVP_Core> * /*core_list*/)
{
	...
	m_fs_f_Rcs.mi2mult3(&m_ws_f_Rcs, &m_fs_f_ws); // #+# wahrscheinlich nur temporaer verwendet
	...
	{ // cardan joint
		...
		m_Ars_f_Acs.get_row(mapping_uRrs_f_Rrs[IVP_INDEX_X], &fixed_axisArs_Acs); // The fixed axis of objectA ...
		IVP_U_Point fixed_axisArs_ws;
		m_ws_f_Acs.vmult3(&fixed_axisArs_Acs, &fixed_axisArs_ws);
		IVP_U_Point fixed_axisArs_Rcs;
		m_ws_f_Rcs.vimult3(&fixed_axisArs_ws, &fixed_axisArs_Rcs); // ... transformed into Rcs
		IVP_U_Point fixed_axisRrs_Rcs;
		m_Rrs_f_Rcs.get_row(mapping_uRrs_f_Rrs[IVP_INDEX_X], &fixed_axisRrs_Rcs);		// The fixed reference axis of objectR in Rcs
		...
		m_rs_f_Rcs_buffer.init_normized3_col(&rsx_Rcs, mapping_uRrs_f_Rrs[IVP_INDEX_X]);
		m_rs_f_Rcs_buffer.transpose3(); // #+# use init normized3_row or program init_normized3_inv
	...
	if (fixedrot_dim + limitedrot_dim == 3)
	{ // same orientation
	...
	else if (fixedrot_dim + limitedrot_dim == 2)
	{ // hinge
	...
	else if (fixedrot_dim + limitedrot_dim == 0)
	{ // free rotation
		drRA_now_rs.set_to_zero();
	}
	...
	IVP_IF(0)
	{
		...
		if (coreR)
			coreR->environment->add_draw_vector(m_ws_f_Rcs.get_position(), &world_vec, m_Rfs_f_Rcs.object->get_name(), 3);
		if (coreA)
			coreA->environment->add_draw_vector(m_ws_f_Acs.get_position(), &world_vec, m_Afs_f_Acs.object->get_name(), 3);
	}
	...
	if (fixedrot_dim == 1)
	{
		...
		IVP_U_Point fixed_axisArs_Acs;
		m_Ars_f_Acs.get_row(mapping_uRrs_f_Rrs[IVP_INDEX_X], &fixed_axisArs_Acs); // Die feste Achse von objectA ...
		IVP_U_Point fixed_axisArs_ws;
		m_ws_f_Acs.vmult3(&fixed_axisArs_Acs, &fixed_axisArs_ws);
		IVP_U_Point fixed_axisArs_Rcs;
		m_ws_f_Rcs.vimult3(&fixed_axisArs_ws, &fixed_axisArs_Rcs); // ... transformiert ins Rcs
		IVP_U_Point fixed_axisRrs_Rcs;
		m_Rrs_f_Rcs.get_row(mapping_uRrs_f_Rrs[IVP_INDEX_X], &fixed_axisRrs_Rcs); // Die feste Referenzachse von objectR im Rcs
	...
	if (maxforce)
	{
		...
		case IVP_NORM_MAXIMUM:
			in_range = IVP_Inline_Math::fabsd(impulseR_fs.k[0]) < maxforce->halfimpulse[0] && IVP_Inline_Math::fabsd(impulseR_fs.k[1]) < maxforce->halfimpulse[1] && IVP_Inline_Math::fabsd(impulseR_fs.k[2]) < maxforce->halfimpulse[2] && IVP_Inline_Math::fabsd(impulserotR_rs.k[0]) < maxforce->halfimpulse[3] && IVP_Inline_Math::fabsd(impulserotR_rs.k[1]) < maxforce->halfimpulse[4] && IVP_Inline_Math::fabsd(impulserotR_rs.k[2]) < maxforce->halfimpulse[5];
			break;
		}
		IVP_IF(0)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				if (maxforce->halfimpulse[i] < simplenorm_transdir)
				{
					...
					case IVP_CFE_BREAK: // constraint breaks in this axis
						...
						IVP_IF(1)
						printf("Translationsachse %d ist gebrochen!\n", i);
						break;
				...
				if (maxforce->halfimpulse[i + 3] / simplenorm_rotdir < 1.0f)
				{
					...
					case IVP_CFE_BREAK:
						...
						IVP_IF(1)
						printf("Rotationsachse %d ist gebrochen!\n", i);
						break;
					case IVP_CFE_BEND:
						// NOT implemented
						break;
...
void IVP_Constraint_Local::change_fixing_point_Ros(const IVP_U_Point *anchor)
{ // point_Ros_f_nRfs, point_nRfs_in_Ros
...
void IVP_Constraint_Local::change_translation_axes_Ros(const IVP_U_Matrix3 *trans_axes)
{ // m_Ros_f_nRfs, m_nRfs_in_Ros
	if (m_Rfs_f_Rcs.object)
		m_Rfs_f_Rcs.object->ensure_in_simulation();
	else
		m_Afs_f_Acs.object->ensure_in_simulation();
...
void IVP_Constraint_Local::change_target_translation_axes_Ros(const IVP_U_Matrix3 *trans_axes)
{ // m_Ros_f_nRfs, m_nRfs_in_Ros
	if (m_Rfs_f_Rcs.object)
		m_Rfs_f_Rcs.object->ensure_in_simulation();
	else
...
void IVP_Constraint_Local::change_rotation_axes_Ros(const IVP_U_Matrix3 *rot_axes)
{ // m_Ros_f_nRrs, m_nRrs_in_Ros
	if (m_Rfs_f_Rcs.object)
		m_Rfs_f_Rcs.object->ensure_in_simulation();
	else
		m_Afs_f_Acs.object->ensure_in_simulation();
...
void IVP_Constraint_Local::change_target_rotation_axes_Ros(const IVP_U_Matrix3 *rot_axes)
{ // m_Ros_f_nRrs
	if (m_Rfs_f_Rcs.object)
		m_Rfs_f_Rcs.object->ensure_in_simulation();
	else
	...
	if (m_Rfs_f_Rcs.object)
		...
		m_Rcs_f_Ros.init();
	IVP_U_Matrix3 m_Rcs_f_nRrs;
	m_Rcs_f_Ros.mmult3(rot_axes, &m_Rcs_f_nRrs); // m_Rcs_f_nRrs inverted is the result
