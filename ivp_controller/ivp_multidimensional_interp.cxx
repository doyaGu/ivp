...
IVP_RETURN_TYPE IVP_Multidimensional_Interpolator::linfit(int m, int n, IVP_MI_Vector **A, IVP_FLOAT *X, IVP_FLOAT *res)
{
	...
	for (int j = 0; j < n; j++)
	{
		...
		if (IVP_Inline_Math::fabsd(A[j]->element[j]) <= f)
			return (IVP_FAULT); // matrix is not invertible (it is singularian), so we cannot substitute back later
	}
																	   ...
																	   IVP_MI_Vector *output)
{
	...
	if (success)
	{ // new_input was successfully interpolated, the weights of the old input vectors involved can be found in interpolation_weights
