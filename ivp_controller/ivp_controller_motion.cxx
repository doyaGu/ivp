...
void IVP_Controller_Motion::do_simulation_controller(IVP_Event_Sim *es, IVP_U_Vector<IVP_Core> *core_list)
{
    ...
    delta_position.inline_subtract_and_mult(&this->target_pos_ws, cur_pos_ws, this->force_factor);
    ...
    // ---------------------------------------------------------
    {
        ...
        delta_quat_core_f_core.set_mult_quat(&target_q_core_f_world, &core->q_world_f_core_next_psi);
        ...
        for (int i = 2; i >= 0; --i)
        {
            ...
            if (IVP_Inline_Math::fabsd(rot_force) < max_torque.k[i])
                continue;
...
{
    if (position->quad_distance_to(&target_pos_ws) == 0)
        return;
...
IVP_Controller_Golem::IVP_Controller_Golem(IVP_Real_Object *o, const IVP_Template_Controller_Golem *t) : IVP_Controller_Motion(o, t)
{
