                                                               ...
                                                               const IVP_U_Float_Point *geom_center_os)
{
    ...
    {
    case 0:
    { // speed_plane_os and/or ball is completely under surface
                                                                ...
                                                                const IVP_U_Float_Point *p2_os)
{
    ...
    break;
    case 1:
    { // center lies above the surface and speed_plane_os disects ball under and above the surface
        ...
        if (factor <= buoyancy_eps)
        { // ball falls into the medium
            ...
            if (surface_content_down < buoyancy_eps)
            { // safety check
            ...
            else
            {
                ...
                if (factor > -buoyancy_eps)
                { // speed_plane_os is orthogonal on surface_os
            ...
            else
            {
                ...
                if (surface_content_up < buoyancy_eps)
                { // safety check
                    center_of_part_ellipse_os.set(0.0f, 0.0f, 0.0f);
    ...
    {
    case_2:
        // center lies above the surface and speed_plane_os disects ball only above the surface
    ...
    case 3:
    {
        // center lies under the surface and speed_plane_os disects ball under and above the surface
        ...
        if (factor <= buoyancy_eps) // ball falls into the medium
        {
            ...
            if (cut_off_part_above_surface < buoyancy_eps)
            { // safety check
        ...
        { // ball flies out of the water
            ...
            if (surface_content_of_circle_segment < buoyancy_eps)
            { // safety check
            ...
            if (surface_content_of_circle_segment_under_surface < buoyancy_eps)
            { // safety check
            ...
            IVP_DOUBLE ellipse_arc = 2 * IVP_Inline_Math::fast_asin(s / (2 * b)) * b;
            IVP_DOUBLE A = 0.5f * (ellipse_arc * b - s * (b - height));
            if ((b < buoyancy_eps) || (A < buoyancy_eps))
            { // safety check
                center_of_ellipse_segment.set(0.0f, 0.0f, 0.0f);
            }
            else
            ...
            if (surface_content_of_ellipse_segment_under_surface < buoyancy_eps)
            { // safety check
    ...
    break;
    case 4:
    { // center lies under surface and speed_plane_os disects completely under surface
    case_4:
                                                      ...
                                                      const IVP_U_Float_Point *rel_speed_of_current_os)
{
    ...
    if (distance <= 0.0f)
    {
        // center lies above surface
        if (IVP_Inline_Math::fabsd(distance) >= radius)
        {           // ball is completely above surface
            return; // nothing to do
        }
        else
        {
            // ball partly under the surface, center lies above
    ...
    else
    {
        ...
        { // ball partly under the surface, center lies under
            int location = compute_disection_points_with_ball(surface_os, &speed_plane_os, &geom_center_os, radius, &p1_os, &p2_os);
            if (location)
            {
                                                                      ...
                                                                      const IVP_U_Float_Point *s_point)
{
    ...
    {
    case 0:
    { // triangle is completely under the surface
...
#endif
    case 1:
    { // the vertex with index "index_neg" is above the surface
    ...
    break;
    case 2:
    { // two vertices are above the surface, the vertex with index "index_positiv" is under the surface
                                                                                   ...
                                                                                   const int &index_neg)
{
    ...
    {
    case 0:
    { // current_triangle is completely under the surface
...

    {
        ...
        if (normal_os.quad_length() < buoyancy_eps)
        { // savety check
                                                       ...
                                                       const IVP_U_Float_Point *s_point)
{
    ...
    for (int i = nr_of_triangles; --i >= 0; triangle = triangle->get_next_tri())
    { // fetch next triangle of ledge
