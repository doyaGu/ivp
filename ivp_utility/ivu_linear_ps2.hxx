...
inline void IVP_U_Quat::inline_set_mult_quat(const IVP_U_Quat *q1, const IVP_U_Quat *q2)
{
  ...
  res->w = ww;
    ...
    asm __volatile__("
                     ...
                     vmul.xyzw vf12,
                     vf4,
                     vf5 #vf12 x1x2 y1y2 z1z2 w1w2
                         vmulw.xyz vf8,
                     vf4,
                     vf5 #vf8 x1w2 y1w2 z1w2 w1w2
                         vmulw.xyz vf9,
                     vf5,
    ...
    #nop
                         vsubx.w vf12,
                     vf12,
                     vf12 #vf12.w = ww - xx vopmula.xyz ACC,
                     vf4,
                     vf5
                         vopmsub.xyz vf6,
                     vf5,
...
void IVP_U_Quat::normize_correct_step(int steps)
{
    ...
    if (steps > 1)
        factor += 0.5f - (factor * factor * square);
    if (steps > 2)
        factor += 0.5f - (factor * factor * square);
    if (steps > 3)
        factor += 0.5f - (factor * factor * square);
