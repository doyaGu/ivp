...
void IVP_U_Quat::set_quaternion(const IVP_U_Matrix3 *mat)
{
    ...
    else
    {
        ...
        if (m.get_elem(1, 1) > m.get_elem(0, 0))
            i = 1;
        if (m.get_elem(2, 2) > m.get_elem(i, i))
            i = 2;
        ...
        IVP_DOUBLE s =
            IVP_Inline_Math::sqrtd(m.get_elem(i, i) - (m.get_elem(j, j) + m.get_elem(k, k)) + 1.0f);

        q[i] = s * 0.5f;

        if (s != 0.0f)
            s = 0.5f / s;  // knappe Abfrage?
...
void IVP_U_Quat::set_interpolate_smoothly(const IVP_U_Quat *from, const IVP_U_Quat *to, IVP_DOUBLE t)
{
    ...
    // calc cosine
    IVP_DOUBLE cosom = from->x * to->x + from->y * to->y + from->z * to->z + from->w * to->w;
...
***********************************************************************EDOC*/
void APIENTRY gluEulerToQuat_EXT(GLIVP_FLOAT roll, GLIVP_FLOAT pitch, GLIVP_FLOAT yaw, GL_QUAT *quat)
{
...
***********************************************************************EDOC*/
void APIENTRY gluQuatGetValue_EXT(GL_QUAT *quat, GLIVP_FLOAT *x, GLIVP_FLOAT *y, GLIVP_FLOAT *z, GLIVP_FLOAT *radians)
{
    GLIVP_FLOAT len;
...
***********************************************************************EDOC*/
void APIENTRY gluQuatSetValue_EXT(GL_QUAT *quat, GLIVP_FLOAT x, GLIVP_FLOAT y, GLIVP_FLOAT z, GLIVP_FLOAT angle)
{
