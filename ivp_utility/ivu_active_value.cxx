...
void IVP_U_Active_Value_Manager::init_active_values_generic()
{
    ...
    // Do not leak if insert fails.
    if (!this->insert_active_float(mod_double_null))
    {
        delete mod_double_null;
    }
...
IVP_U_Active_Float *IVP_U_Active_Value_Manager::get_active_float_by_name(const char *i_name)
{
    ...
    int pos = 0;
    if (i_name[pos] == '-')
        pos++;  // for negative sign
    if (i_name[pos] == '.')
        pos++;  // for omitted leading zero
...
IVP_U_Active_Sine::IVP_U_Active_Sine(const char *i_name,
                                     ...
                                     IVP_DOUBLE i_time_shift) : IVP_U_Active_Float(i_name)
{
...
{
    if (this->last_update == this->change_meter)
