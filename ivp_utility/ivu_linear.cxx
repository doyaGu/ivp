...
void IVP_U_Point::solve_quadratic_equation_fast(const IVP_U_Point *p)
{  // points stores quadratic equ. k[2] + x*k[1] * x^2 * k[0]
...
void IVP_U_Point::solve_quadratic_equation_accurate(const IVP_U_Point *p)
{  // points stores quadratic equ. k[2] + x*k[1] * x^2 * k[0]
    if (IVP_Inline_Math::fabsd(p->k[0]) < P_DOUBLE_EPS)
    {  // linear equation
        k[0] = 0;
...
int IVP_U_Matrix3::calc_eigen_vector(IVP_DOUBLE eigen_value, IVP_U_Point *eigen_vector_out)
{
    ...
    // get index of pivot element in 1 or 2rd row->flip rows
    {
        ...
        if (max_col != 1)
        {  // switch columns 1 and 2 and index pointer
            IVP_DOUBLE h;
            h = m[0][1];
            m[0][1] = m[0][2];
            m[0][2] = h;
...
void IVP_U_Float_Hesse::calc_hesse(const IVP_U_Float_Point *tp0, const IVP_U_Float_Point *tp1, const IVP_U_Float_Point *tp2)
{
    ...
    this->hesse_val = (IVP_FLOAT) - (this->k[0] * fp0 + this->k[1] * fp1 + this->k[2] * fp2);
};
...
void IVP_U_Matrix::interpolate(const IVP_U_Matrix *m1, const IVP_U_Matrix *m2, IVP_DOUBLE i_factor)
{
...
void IVP_U_Matrix::init_rot_multiple(const IVP_U_Point *angles, IVP_DOUBLE factor)
{  // angles.k[0] = beta, [1] = gamma [2] = alpha,
...
IVP_ERROR_STRING IVP_U_Matrix::write_to_file(FILE *fp, const char *key)
{
    ...
    fprintf(fp, "\t\t\tMATRIX_ROT %g %g %g %g %g %g %g %g %g\n", this->get_elem(0, 0), this->get_elem(0, 1), this->get_elem(0, 2), this->get_elem(1, 0), this->get_elem(1, 1), this->get_elem(1, 2), this->get_elem(2, 0), this->get_elem(2, 1), this->get_elem(2, 2));
...
IVP_DOUBLE IVP_U_Matrix3::quad_rot_distance_to(const IVP_U_Matrix3 *m2)
{
    ...
    IVP_DOUBLE sum = this->rows[0].quad_distance_to(&m2->rows[0]) + this->rows[1].quad_distance_to(&m2->rows[1]) + this->rows[2].quad_distance_to(&m2->rows[2]);
    return sum;
}
