...
void IVP_GridBuilder_Array::add_triangle(int s_p0_off, int s_p1_off, int s_p2_off, int opp0, int opp1, int opp2)
{
...
void IVP_GridBuilder_Array::convert_strip_to_compact_ledges(int row, IVP_U_Vector<IVP_Compact_Ledge> *ledges)
{
    ...
    check_flat_triangles:
        for (ep = start_pos; ep < strip_size - 3; ep += 2)
        {
            ...
            {
                IVP_DOUBLE diff = height_field[strip_field_offset[p + 0]] + height_field[strip_field_offset[p - 1]] - height_field[strip_field_offset[p - 2]] - height_field[strip_field_offset[p + 1]];
            ...
            if (p == -1)
                break;  // no three flat triangles found
        }
        // now ep is the first point needed
        dest_stripe[dest_index++] = strip_field_offset[ep];  // finalize triangle

        if (ep < strip_size - 1)
        {  // check the next triangle for convex piece
            IVP_DOUBLE diff = height_field[strip_field_offset[ep + 0]] + height_field[strip_field_offset[ep - 1]] - height_field[strip_field_offset[ep - 2]] - height_field[strip_field_offset[ep + 1]];
            if (diff >= 0.0f)
            {  // convex edge
                start_pos = ep + 1;
...
IVP_Compact_Grid *IVP_GridBuilder_Array::compile_ledges_into_compact_grid(const IVP_Template_Compact_Grid *gp, IVP_U_Vector<IVP_Compact_Ledge> *ledges)
{
    ...
    {  // find center, use bounding box
        ...
        {
            if (height_field[x] > max)
                max = height_field[x];
            if (height_field[x] < min)
                min = height_field[x];
        }
    ...
    // TL: align offset to next 16 Byte Alignment (I didn't write the GridBuilder, so I have no idea whether the memory will be enough)
