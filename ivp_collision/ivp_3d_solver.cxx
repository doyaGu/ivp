...
IVP_Time IVP_3D_Solver::calc_nullstelle(IVP_Time t0, IVP_Time t1, IVP_DOUBLE value, IVP_DOUBLE v0, IVP_DOUBLE v1, IVP_Real_Object *solver_a, IVP_Real_Object *solver_b)
{
    ...
    while (1)
    {
        ...
        // modified midpoint
        if ((counter & 0x3) == 3)
        {  // sometimes split interval int
            if (counter > 64)
                return t0;                   // endless loop, take left value
        ...
        // 	printf("%i %G	%G:%G	%G:%G	%G:%G    %G\n", counter,value,  t0.get_time(),v0,
        // tt.get_time(),vv, t1.get_time(),v1, vv-value);
...
IVP_BOOL IVP_3D_Solver::find_first_t_for_value_max_dev(IVP_DOUBLE value, IVP_Time t_now, IVP_Time t_max, int t_now_cache_index,
                                                       ...
                                                       IVP_Time *t_out)
{
    ...
    while (1)
    {
        ...
        int int_t_step =
            (int)(t_step * (IVP_3D_SOLVER_MAX_STEPS_PER_PSI * IVP_3D_SOLVER_PSIS_PER_SECOND));
        ...
        IVP_DOUBLE d_tstep =
            int_t_step * (1.0f / (IVP_3D_SOLVER_MAX_STEPS_PER_PSI * IVP_3D_SOLVER_PSIS_PER_SECOND));
        IVP_Time t1 = t0;
        t1 += d_tstep;
        ...
        if (cache1_index == IVP_3D_SOLVER_MAX_STEPS_PER_PSI)
            return IVP_FALSE;
        t0 = t1;
        v0 = v1;
    }
...
IVP_BOOL IVP_3D_Solver::find_first_t_for_value_max_dev2(IVP_DOUBLE value, IVP_Time t_now, IVP_Time t_max, int t_now_cache_index, IVP_U_Matrix_Cache *mc_A, IVP_U_Matrix_Cache *mc_B, IVP_DOUBLE *opt_val_at_t_now, IVP_Time *t_out)
{
    return find_first_t_for_value_max_dev(value, t_now, t_max, t_now_cache_index, mc_A, mc_B, opt_val_at_t_now, t_out);
}

#if 0
/** we know that there will be no zero value for first deviation !!! */
IVP_BOOL IVP_3D_Solver::find_first_t_for_value_no_zero_dev(IVP_DOUBLE value, IVP_DOUBLE t_now, IVP_DOUBLE t_max, int t_now_cache_index, IVP_U_Matrix_Cache *mc_A, IVP_U_Matrix_Cache *mc_B, IVP_DOUBLE *opt_val_at_t_now, IVP_DOUBLE *opt_val_at_next_psi, IVP_DOUBLE *t_out)
{
    /***** first 'null'-stelle from t_now to t_max *******/
    ...
    else
    {
        ...
        v1 = get_value(A, B);
        if (v1 >= value)
            return IVP_FALSE;  // no zero value
    }
    // ok we know that there will be a zero value

    if (t_max != mc_A->core->time_of_next_psi)
    {  // check more in more detail
        IVP_U_Matrix A, B;
...
IVP_BOOL IVP_3D_Solver::find_first_t_for_value_coll(IVP_DOUBLE value, IVP_DOUBLE value2, IVP_Time t_now, IVP_Time t_max, IVP_U_Matrix_Cache *mc_A, IVP_U_Matrix_Cache *mc_B, IVP_DOUBLE *opt_val_at_t_now, IVP_Time *t_out)
{
    /***** first 'null'-stelle from t_now to t_max *******/
    ...
    while (t0 - t_max < 0.0f)
    {
        ...
        if (v1 > value)
        {
            ...
            return find_first_t_for_value_max_dev(value, t1, t_max, cache1_index, mc_A, mc_B, &t1_val, t_out);
        }
        if (v1 <= tnow_val)
        {
