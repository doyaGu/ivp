...
{
    if (!IVP_DEBUG_OBJECT0)
        return IVP_FALSE;
    if (!IVP_DEBUG_OBJECT1)
        return IVP_FALSE;
...
IVP_DOUBLE IVP_Mindist_Minimize_Solver::p_optimize_FF(const IVP_Compact_Edge *A, const IVP_Compact_Edge *B, IVP_Cache_Ledge_Point *m_cache_A, IVP_Cache_Ledge_Point *m_cache_B, IVP_DOUBLE min_qdist)
{
...
IVP_RETURN_TYPE IVP_Mindist::validate_termination_len(IVP_DOUBLE now_len)
{
    ...
    if (now_len - IVP_MIN_TERMINATION_QDLEN > t_len)
    {  // @@@ test for '=='
        CORE;
    }
    if (now_len - IVP_MIN_TERMINATION_QDLEN_EPS > t_len)
    {  // @@@ test for '=='
