...
IVP_Ray_Solver::IVP_Ray_Solver(const IVP_Ray_Solver_Template *templ)
{
    ...
    IVP_ASSERT((templ->ray_normized_direction.real_length() > (1.0f - 1e-4f)) && (templ->ray_normized_direction.real_length() < (1.0f + 1e-4f)));
...
void IVP_Ray_Solver_Min_Hash::add_hit_object(IVP_Real_Object *object, const IVP_Compact_Ledge *compact_ledge, const IVP_Compact_Triangle *compact_triangle, IVP_DOUBLE hit_dist, IVP_U_Point *hit_sur_vec_os)
{
...
IVP_BOOL IVP_Ray_Solver::check_ray_against_sphere(const IVP_U_Float_Point *sphere_center_ws, IVP_FLOAT sphere_radius)
{
    ...
    if (quad_dist < coll_dist * coll_dist)
        return IVP_TRUE;
                                                  ...
                                                  int coord_1)
{
    ...
    for (i = 1; i >= 0; --i)
    {
        ...
        if ((comp_val < (min_coords->k[coord] - ray_start_point.k[coord]) * pos_axis_len) || (comp_val > (max_coords->k[coord] - ray_start_point.k[coord]) * pos_axis_len))
        {
            // miss
            return IVP_FALSE;
        }
...
IVP_BOOL IVP_Ray_Solver::check_ray_against_cube(const IVP_U_Float_Point *luf_point, const IVP_U_Float_Point *rlb_point)
{
...
IVP_BOOL IVP_Ray_Solver_Group::check_ray_group_against_cube(const IVP_U_Float_Point *cube_center_ws, IVP_FLOAT cube_size)
{
    ...
    if (dist.k[0] > cube_size)
        return IVP_FALSE;
    if (dist.k[1] > cube_size)
        return IVP_FALSE;
    if (dist.k[2] > cube_size)
        return IVP_FALSE;
    if (dist.quad_length() >= cube_size * cube_size * 3)
        return IVP_FALSE;
