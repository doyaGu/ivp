...
void IVP_Mindist_Event_Solver::calc_next_event_PF(const IVP_Compact_Edge *P, const IVP_Compact_Edge *F, IVP_Cache_Ledge_Point *m_cache_P, IVP_Cache_Ledge_Point *m_cache_F)
{
                                                  ...
                                                  IVP_Cache_Ledge_Point *m_cache_F)
{
    ...
    IVP_3D_Solver_PF_COLL pf_coll_solver;
    {
        ...
        IVP_BOOL result = pf_coll_solver.find_first_t_for_value_coll(sec_dist, real_coll, t_now, t_max, &cache_P, &cache_F, &now_len, &event_time_out);
        if (result)
        {
            event_type_out = IVP_COLL_PF_COLL;
        }
                                                  ...
                                                  IVP_Cache_Ledge_Point *m_cache_L)
{
    ...
    /** detect collision time, first **/
    {
        ...
        IVP_DOUBLE found =
            solver_kk_coll.find_first_t_for_value_coll(mindist->get_coll_dist(), ivp_mindist_settings.real_coll_dist, t_now, event_time_out, &cache_K, &cache_L, 0, &event_time_out);
        if (found)
        {
                                                  ...
                                                  IVP_Cache_Ledge_Point *m_cache_P2)
{
    ...
    // check collision of both points
    {
        ...
        IVP_IF(1)
        {
            ...
            IVP_CLS.give_world_coords_AT(P, m_cache_P, &p_world);
            const IVP_U_Point p2_world;
            IVP_CLS.give_world_coords_AT(P2, m_cache_P2, &p2_world);
    ...
    // check s vals for sector change
    {
        ...
        for (int j = 0; j < 2; j++)
        {
            ...
            for (const IVP_Compact_Edge *e = point2->get_prev()->get_opposite(); 1; e = e->get_prev()->get_opposite())
            {
                ...
                if (solver_pp_pk.find_first_t_for_value_max_dev(sec_dist, t_now, event_time_out, 0, cache[j], cache[1 - j], 0, &event_time_out))
                {
...
/***************************** PP ******************************************/
/***************************** PP ******************************************/
void IVP_Mindist_Event_Solver::calc_next_event_BP(IVP_Ball *ball, const IVP_Compact_Edge *P2, IVP_Cache_Object *m_cache_B, IVP_Cache_Ledge_Point *m_cache_P2)
{
    // returns the time till the next event
    // if real_event_occurred is false, no event until max_delta_time occurred
    IVP_U_Matrix_Cache cache_P(m_cache_B);
    ...
    // check s vals for sector change
    {
...

        {
            ...
            for (const IVP_Compact_Edge *e = P2->get_prev()->get_opposite(); 1; e = e->get_prev()->get_opposite())
            {
                ...
                if (solver_pp_pk.find_first_t_for_value_max_dev(sec_dist, t_now, event_time_out, 0, &cache_P, &cache_P2, 0, &event_time_out))
                {
                    event_type_out = IVP_COLL_PP_PK;
                }
                if (e == P2)
...
/***************************** BB ******************************************/
/***************************** BB ******************************************/
void IVP_Mindist_Event_Solver::calc_next_event_BB(IVP_Cache_Object *m_cache_A, IVP_Cache_Object *m_cache_B)
{
    // returns the time till the next event
    // if real_event_occurred is false, no event until max_delta_time occurred
                                                  ...
                                                  IVP_Cache_Ledge_Point *m_cache_K)
{
    ...
    // both areas F0/F1
    {
        ...
        for (i = 0; i <= 1; i++)
        {
            ...
            ivp_u_bool found_flag =
                solver_pk_pf.find_first_t_for_value_max_dev(sec_dist, t_now, event_time_out, 0, &cache_P, &cache_K, 0, &event_time_out);
            if (found_flag)
            {
    ...
    IVP_DOUBLE max_dev;
    {
        ...
        if (mindist_PK < P_RES_EPS)
            mindist_PK = P_RES_EPS;
...

    {
        ...
        for (const IVP_Compact_Edge *e = P->get_prev()->get_opposite(); 1; e = e->get_prev()->get_opposite())
        {
            ...
            ivp_u_bool found_flag =
                solver_pk_kk.find_first_t_for_value_max_dev(sec_dist, t_now, event_time_out, 0, &cache_P, &cache_K, 0, &event_time_out);
            if (found_flag)
            {
...
/***************************** PK ******************************************/
/***************************** PK ******************************************/
void IVP_Mindist_Event_Solver::calc_next_event_BK(IVP_Ball *ball, const IVP_Compact_Edge *K, IVP_Cache_Object *m_cache_B, IVP_Cache_Ledge_Point *m_cache_K)
{
    IVP_U_Matrix_Cache cache_P(m_cache_B);
    IVP_U_Matrix_Cache cache_K(m_cache_K->clp_cache_object);
