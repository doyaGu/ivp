...
void IVP_Clustering_Visualizer_Shortrange::add_node_to_pipeline(const IVP_Compact_Ledgetree_Node *node, IVP_BOOL reported_ledge)
{
    ...
    if (node->is_terminal())
        ...
        new_nodedata->n_touches = 0;  // no reference counting for non-terminal nodes!
...
IVP_BOOL IVP_Clustering_Visualizer_Shortrange::recursively_traverse_cluster(const IVP_Compact_Ledgetree_Node *node)
{
...

    {
        ...
        if (IVP_Inline_Math::fabsd(distance_between_centers.k[0]) >= (node->box_sizes[0] * work + this->intruder_radius))
            goto abort;
        if (IVP_Inline_Math::fabsd(distance_between_centers.k[1]) >= (node->box_sizes[1] * work + this->intruder_radius))
            goto abort;
        if (IVP_Inline_Math::fabsd(distance_between_centers.k[2]) >= (node->box_sizes[2] * work + this->intruder_radius))
            goto abort;
    }
...
abort:
    if (this->boxmode != TERMINALS_ONLY)
    {  // display for ABORTED_BOXES and TRAVERSED_BOXES
        this->add_node_to_pipeline(node, IVP_FALSE);
    }
    return (IVP_TRUE);
}
    ...
    for (i = 0; i < this->output_callbacks.len(); i++)
    {
        ...
        for (x = this->pipelined_objects.len() - 1; x >= 0; x--)
        {
            ...
            for (y = objectdata->ledgetree_nodes.len() - 1; y >= 0; y--)
            {
                ...
                if (callback->reported_ledge)
                    this->stats_n_reported_ledges++;
...
void IVP_Clustering_Visualizer_Shortrange::analyze_collision(IVP_Real_Object *object0, IVP_DOUBLE radius0, IVP_Real_Object *object1, IVP_DOUBLE radius1)
{
    ...
    for (x = 0; x < 2; x++)
    {
        ...
        if (this->use_intruder_accept_filter && !this->accept_filter_intruder->find(this->intruder))
            continue;  // only process intruder objects that are mentioned in the filter!
        if (this->use_intruder_reject_filter && this->reject_filter_intruder->find(this->intruder))
            continue;  // only process intruder objects that are mentioned in the filter!
        if (this->use_private_property_accept_filter && !this->accept_filter_private_property->find(this->private_property))
            continue;  // only process private_property objects that are mentioned in the filter!
        if (this->use_private_property_reject_filter && this->reject_filter_private_property->find(this->private_property))
            continue;  // only process private_property objects that are mentioned in the filter!
...
void IVP_Clustering_Visualizer_Shortrange::add_object_to_intruder_filter(IVP_Real_Object *obj, IVP_CLUSTERING_VISUALIZER_FILTER filter)
{
    switch (filter)
    {
        case ACCEPT:
        {
            if (!this->accept_filter_intruder->find(obj))
            {                                            // pair already stored?
        ...
        case REJECT:
        {
            if (!this->reject_filter_intruder->find(obj))
            {                                            // pair already stored?
...
void IVP_Clustering_Visualizer_Shortrange::add_object_to_private_property_filter(IVP_Real_Object *obj, IVP_CLUSTERING_VISUALIZER_FILTER filter)
{
    switch (filter)
    {
        case ACCEPT:
        {
            if (!this->accept_filter_private_property->find(obj))
            {                                                    // pair already stored?
        ...
        case REJECT:
        {
            if (!this->reject_filter_private_property->find(obj))
            {                                                    // pair already stored?
...
void IVP_Clustering_Visualizer_Longrange::remove_objectdata(IVP_Real_Object *real_object)
{
    ...
    {
        IVP_Clustering_Visualizer_Longrange_Data *data = this->visualize_data.element_at(x);
        if (data->real_object != real_object)
            continue;
        this->visualize_data.remove(data);
        P_DELETE(data);
        return;
...
void IVP_Clustering_Visualizer_Longrange::add_object_to_filter(IVP_Real_Object *obj, IVP_CLUSTERING_VISUALIZER_FILTER filter)
{
    switch (filter)
    {
        case ACCEPT:
        {
            if (!this->accept_filter->find(obj))
            {                                   // pair already stored?
        ...
        case REJECT:
        {
            if (!this->reject_filter->find(obj))
            {                                   // pair already stored?
