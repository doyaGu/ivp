...
void IVP_Intrusion_Intersection::print(const char *text)
{
	...
	{
		if (intp(pairing_intersection) > intp(this))
			return;
	}
...
int IVP_Tetra_Intrude::pe(const int pnt_num)
{ // print all edges including pnt_num
	int i;
												  ...
												  IVP_Intrusion_Status *status)
{
	...
	while (1)
	{
		if (((bitsand & 0xf & ~(bitsand >> 8)) & (1 << me)) != 0)
			return IVP_INTRUSION_CHECK_NONE; // both both inside and not on the surface

		if ((bor & (0x1111 << me)) == (0x11 << me))
		{ /* real intrusion, no epsilon, no identic to point ->
one inside, one outside*/
		...
		if ((t0->tmp_side_of_triangle_bits & (1 << (me + 12))) != 0)
		{ // identic to point
		...
		if ((t1->tmp_side_of_triangle_bits & (1 << (me + 12))) != 0)
		{ // identic to point
		...
		if ((t0->tmp_side_of_triangle_bits & (1 << (8 + me))) != 0)
		{ // mid of a surface
			intrude_position.set(t0->opoint);
			break;
		}
		if ((t1->tmp_side_of_triangle_bits & (1 << (8 + me))) != 0)
		{ // mid of a surface
	...
	if (dist1 < -pop_eps_of_t[other1])
		return IVP_INTRUSION_CHECK_NONE;
	if (dist2 < -pop_eps_of_t[other2])
		return IVP_INTRUSION_CHECK_NONE;

	{
		...
		for (int i = 5; i >= 0; i--)
		{
			...
			if (dist < meps)
			{														 // now collision point is outside eps of edges
										 ...
										 IVP_Intrusion_Status *status)
{
	...
	if ((tp->tmp_side_of_triangle_bits & (0xf << 20)) != 0)
	{ // one side really outside
		return IVP_INTRUSION_CHECK_NONE;
	}
	{
		for (int i = 5; i >= 0; i--)
	...
	if ((tp->tmp_side_of_triangle_bits & 0xf) != 0xf)
	{ // not within DOUBLE EPS -> on the surface
		return IVP_INTRUSION_CHECK_EPSILON;
	}
	if (!status)
		return IVP_INTRUSION_CHECK_POINT;
															   ...
															   IVP_Intrusion_Status *status)
{
	   ...
	   ok -> check hesse to 4 planes ***** */
	{
		...
		for (pnr = n_tetra_points - 1; pnr >= 0; pnr--, tp++)
		{
			...
			if (s > result)
				result = s;
			if (result > IVP_INTRUSION_CHECK_EPSILON)
				return result;
		}
		{
			IVP_Extra_Point *op;
	   ...
	   tmp.tetra_point bits, then real check ****/
	{
		...
		for (tnr--; tnr >= 0; te++, tnr--)
		{
			...
			if ((bitsand & 0xf0) != 0)
				continue; // both points real outside
			if ((bitsand & (1 << 16)))
			{
				continue;
			}
