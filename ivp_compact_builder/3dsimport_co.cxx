        ...
        if (argv[n][0] == '-' || argv[n][0] == '/')
        {
            switch (toupper(argv[n][1]))
            {
                case 'O':
                    ...
                    {
                        ivp_message("Missing object name!\n");
                        return 1;
                    }
                case 'S':
                    if (n + 3 < argc)
                    ...
                    {
                        ivp_message("Missing scale value!\n");
                        return 1;
                    }
    ...
    for (i = 0; i < scene->meshobjs; i++)
    {
        ...
        for (x = 0; x < (int)mo->faces;)
        {
            ...
            IVP_Compact_Ledge *compact_ledge =
                IVP_SurfaceBuilder_Pointsoup::convert_pointsoup_to_compact_ledge(&polygon);
            if (compact_ledge)
            {
