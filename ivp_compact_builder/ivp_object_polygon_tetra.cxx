...
{
    int i;
    if (!this->triangle->other_side)
        return NULL;
    IVP_Tri_Edge *e = &this->triangle->other_side->three_edges[0];
    for (i = 2; i >= 0; i--)
    {
        // MoeMod : prevent NPE
        if (e->next && e->next->start_point == this->start_point)
            return e;
        e++;
    }
    return NULL;
...
int P_Sur_2D_Line::has_points(P_Sur_2D_Point *point_a, P_Sur_2D_Point *point_b)
{
    if (!point_b)
    {
        if ((start_point == point_a) || (end_point == point_a))
            return 1;
        return 0;
    }

    if ((start_point == point_a) && (end_point == point_b))
        return 1;
    if ((end_point == point_a) && (start_point == point_b))
        return 1;
    return 0;
}
...
#endif
    if (max_koord == 1)
        trafo_is_mirrored = 1 - trafo_is_mirrored;
...
IVP_ERROR_STRING P_Sur_2D::calc_triangle_representation()
{
    ...
    while ((td_base_line = this->lines.first))
    {
...
#endif
        if (loop_counter++ > 100)
        { //@@@@SF: changed from 100000 to 100 [05.03.2000]
        ...
        {
            if (td_point_line == td_base_line)
                continue;
            if (td_point_line->start_point == td_base_line->end_point)
                continue;
#ifdef SUR_DEBUG
            ...
            {
                // crosses ca ?
                if (!td_cross_line->has_points(point_c, point_a))
                { // !'identical'
                ...
                // crosses bc ?
                if (!td_cross_line->has_points(point_b, point_c))
                { // !'identical'
...
IVP_ERROR_STRING IVP_Object_Polygon_Tetra::final_convexify_check()
{
    ...
    for (tri = ph->triangles.first; tri; tri = tri->next)
    {
        ...
        for (i = 0; i < 3; i++, e = e->next)
        {
            ...
            {
                error_flag = "error";
                printf("convex_test: tri->edge: tmp.gen.concav_flag == -2 (partly overlapping)");
                tri->print("\n");
            }
            if (conc_flag == -1)
            {
                error_flag = "error";
                printf("convex_test: tri->edge: tmp.gen.concav_flag == -1 (identic)");
                tri->print("\n");
            }
            if (conc_flag == 0)
            {
                ...
                {
                    printf("convex_test:  warning edge is convex, but neighbours are hidden.");
                    tri->print("\n");
                }
            }
            if (conc_flag == 1)
            {
                    ...
                    !e->triangle->flags.is_hidden)
                {
                    ...
                    {
                        printf("convex_test:  edge is concav %f, but neighbours are unhidden.", e->concavity);
                        tri->print("\n");
                        error_flag = "error";
                    }
                }
...
IVP_ERROR_STRING IVP_Object_Polygon_Tetra::check_konsistency_of_triangles()
{
    ...
    for (tri = ph->triangles.first; tri; tri = tri->next)
    {
        ...
        {
            error_flag = "error";
            printf("physic_test: tri != tri->other_side->other_side in triangle ");
            tri->print("\n");
        }
        ...
        {
            error_flag = "error";
            printf("physic_test: edges do not properly form a ring in triangle ");
            tri->print("\n");
        }
        if (tri->calc_areasize() < 1E-6f)
        {
            error_flag = "error";
            printf("pysic_test: tri->calc_areasize() < 1E-6f in triangle: %g", tri->calc_areasize());
            tri->print("\n");
        }
        ...
        if (tri->flags.is_hidden)
        {
            ...
            {
                error_flag = "error";
                printf("physic_test:  triangle is hidden, but neighbours don't form a tetraeder.");
                tri->print("\n");
            }
            ...
            {
                error_flag = "error";
                printf("physic_test:  triangle is hidden, but not all neighbours are hidden.");
                tri->print("\n");
            }
        }
        else
        {
            ...
            {
                error_flag = "error";
                printf("physic_test:  triangle is not hidden, but a neighbour is hidden.");
                tri->print("\n");
            }
        }
        ...
        for (i = 0; i < 3; i++, edge = edge->next)
        {
            ...
            {
                error_flag = "error";
                printf("physic_test: tri->is_hidden && in hash");
                tri->print("\n");
            }
            ...
            {
                error_flag = "error";
                printf("physic_test: edge->opposite->opposite != edge in triangle ");
                edge->print("\n");
            }
            if (edge->start_point != edge->opposite->next->start_point)
            {
                error_flag = "error";
                printf("physic_test: edge->start_point != edge->opposite->next->startpoint in triangle ");
                edge->print("\n");
            }
            if (edge != edge->other_side()->other_side())
            {
                error_flag = "error";
                printf("physic_test: edge != edge->other_side()->other_side() in triangle ");
                edge->print("\n");
            }
            if (edge->prev->start_point == edge->opposite->prev->start_point)
            {
                error_flag = "error";
                printf("physic_test: edge->prev->sp == edge->opp->prev->sp in triangle ");
                edge->print("\n");
            }
            if (edge->triangle == edge->opposite->triangle)
            {
                error_flag = "error";
                printf("physic_test: edge->triangle == edge->opposite->triangle in triangle ");
                edge->print("\n");
            }
            if (edge->behind->other_side()->behind != edge->other_side())
            {
                error_flag = "error";
                printf("physic_test: edge->behind->other_side()->behind = edge->other_side in triangle ");
                edge->print("\n");
            }
            if (edge->triangle != tri)
            {
                error_flag = "error";
                printf("physic_test: edge->triangle != tri in triangle ");
                edge->print("\n");
            }
            if ((edge->behind->start_point != edge->start_point) || (edge->behind->next->start_point != edge->next->start_point))
            {
                error_flag = "error";
                printf("physic_test: (edge->behind->sp != edge->sp) || (edge->behind->next->sp != edge->next->sp) in triangle ");
                tri->print("\n");
            }
        }
    }
...
void IVP_Object_Polygon_Tetra::calc_concavities()
{
    ...
    for (tri = this->triangles.first; tri; tri = tri->next)
    {
        ...
        {
            if ((edge->concavity == uninit) && (!edge->triangle->flags.is_hidden) && (!edge->opposite->triangle->flags.is_hidden))
            {
...
void IVP_Object_Polygon_Tetra::move_edge_to_problem_hash(IVP_Tri_Edge *edge)
{
    ...
    if (opp < edge)
        edge = opp;
    if (edge->triangle->flags.is_hidden)
        return;
...
void IVP_Object_Polygon_Tetra::pop_problematic_edge(IVP_Tri_Edge *edge)
{
    ...
    if (!i_got_a_real_problem && edge->tmp.gen.concav_flag != 0)
    { // check flatness of concav edges
...
void IVP_Object_Polygon_Tetra::record_intruding_convex_edges(IVP_Intrusion_Status *status)
{
    ...
    for (; is; is = is->next)
    {
        ...
        if (typ != IVP_INTRUSION_CHECK_LINE)
            continue;
...
void IVP_Object_Polygon_Tetra::insert_pierce_info()
{
    ...
    // insert all pierce infos
    {
        ...
        {
            if (tri->flags.is_hidden)
                continue; // works at convex surface only
